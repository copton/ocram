...
module Ocram.Backend.CreateEStacks
-- exports {{{1
(
  create_estacks
) where
...
import Ocram.Intermediate.Representation (Function(..), fun_name, var_decl)
...
create_estacks :: CallGraph -> M.Map Symbol Function -> ([CDecl], M.Map Symbol (Maybe CDecl))
...
estackInstance :: CallGraph -> M.Map Symbol Function -> Function -> Maybe CDecl
estackInstance cg funs fun
  ...
  where
    ...
    frames = mapMaybe (frame . $fromJust_s . flip M.lookup funs) ($fromJust_s . call_order cg . fun_name $ fun)
...
estackFrame :: Function -> Maybe CDecl
estackFrame fun
