...
-- imports {{{1
import Ruab.Backend.Representation
import Ruab.Test.Lib (enumTestGroup, paste)
import Test.Framework (Test, testGroup)
import Test.HUnit ((@=?), Assertion)

tests :: Test -- {{{1
tests = testGroup "Backend" [test_render_input, test_parse_output]

test_render_input :: Test -- {{{2
test_render_input = enumTestGroup "render_input" $ map runTest [
    (
  ...
  where
    ...
    runTest (cmd, expected) = expected @=? render_input cmd
