...
expect = [
    ...
    [
        ...
        {"type":"call", "seq":2, "fun":"RPC_READ_FAST_SENSOR", "sensor":1},
        {"type":"response", "seq":2, "fun":"RPC_READ_FAST_SENSOR", "value":-1},
        {"type":"response", "seq":1, "fun":"RPC_TELL", "call":{"type":"response", "seq":0, "fun":"RPC_READ_SLOW_SENSOR", "value":-1}},
    ], [
        {"type":"call", "seq":1, "fun":"RPC_TELL", "peer":"fe80::212:7403:3:303", "call":{"type":"call", "seq":0, "fun":"RPC_READ_SLOW_SENSOR", "sensor":1}},
        {"type":"call", "seq":2, "fun":"RPC_READ_FAST_SENSOR", "sensor":1},
        {"type":"response", "seq":2, "fun":"RPC_READ_FAST_SENSOR", "value":-1},
        ...
        {"type":"response", "seq":0, "fun":"RPC_READ_SLOW_SENSOR", "value":-1},
    ]
    ]
...
def verify(logs):
    ...
    for line in logs:
        ...
        if log != None:
            _, node, action = log
            if node == 2 and time[node] == 2:
                expect[2][2]["value"] = action["value"]
                expect[1][2]["value"] = action["value"]
            if node == 3 and time[node] == 1:
                expect[3][1]["value"] = action["value"]
                expect[2][3]["call"]["value"] = action["value"]
                expect[1][3]["call"]["value"] = action["value"]

            assert action == expect[node][time[node]], "\naction=%s\nexpect=%s\nnode=%d\ntime=%d" % (action, expect[node][time[node]], node, time[node])
            time[node] += 1

            if node == 1 and time[node] == 4:
                loop += 1
                time = [-1, 0, 0, 0]
