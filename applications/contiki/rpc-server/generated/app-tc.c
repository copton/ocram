...
typedef struct {
    bool busy;
    condition_t cond_server;
    condition_t cond_worker;
...
void handle_call(worker_t* worker)
{
    ...
    if (0) { 
        ...
        tc_sleep(clock_time() + SLOW_SENSOR_DELAY);
...
TC_RUN_THREAD void worker0()
{
    ...
    tc_condition_init(&worker->cond_worker);
    tc_condition_init(&worker->cond_server);

    while(1) {
        tc_condition_wait(&worker->cond_worker);
        handle_call(worker);
        tc_condition_signal(&worker->cond_server);
    }
}
