...
void handle_call(worker_t* worker)
{
    ...
    if (0) { 
        ...
        tl_sleep(clock_time() + (CALL->data.read_slow_sensor.sensor * SLOW_SENSOR_DELAY), NULL);
...
void tl_app_main()
{
    ...
    tl_create_thread(worker2, stack_worker2, sizeof(stack_worker2));
    tl_create_thread(worker3, stack_worker3, sizeof(stack_worker3));
