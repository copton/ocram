...
PT_THREAD(worker_thread(worker_t* worker, process_event_t ev)) {
    ...
    if (0) { 
        ...
        etimer_set(&worker->et, CALL->data.read_slow_sensor.sensor * SLOW_SENSOR_DELAY);
...
PROCESS(worker2, "worker2");
PROCESS(worker3, "worker3");
AUTOSTART_PROCESSES(&server_process, &worker0, &worker1, &worker2, &worker3);
...
PROCESS_THREAD(worker2, ev, data)
{
    worker_t* worker = workers + 2;

    PROCESS_BEGIN();

    worker->conn = udp_new(NULL, 0, NULL);
    ASSERT(worker->conn != NULL);
    udp_bind(worker->conn, UDP_CLIENT_PORT + 2);
...
PROCESS_THREAD(worker3, ev, data)
{
    worker_t* worker = workers + 3;

    PROCESS_BEGIN();

    worker->conn = udp_new(NULL, 0, NULL);
    ASSERT(worker->conn != NULL);
    udp_bind(worker->conn, UDP_CLIENT_PORT + 3);
...
PROCESS_THREAD(server_process, ev, data)
{
    ...
    workers[2].process = &worker2;
    workers[3].process = &worker3;
