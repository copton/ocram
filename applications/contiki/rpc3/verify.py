...
expect = [
    ...
    [
        ...
        {"type":"call", "seq":3, "fun":"RPC_READ_FAST_SENSOR", "sensor":1},
        {"type":"response", "seq":3, "fun":"RPC_READ_FAST_SENSOR", "value":2309},
...
def verify(logs):
    ...
    for line in logs:
        ...
        if log != None:
            _, node, action = log
            if node == 2 and time[node] == 3:
                expect[2][3]["value"] = action["value"]
                expect[1][3]["value"] = action["value"]
            if node == 2 and time[node] == 5:
                expect[2][5]["value"] = action["value"]
                expect[1][5]["value"] = action["value"]
            if node == 3 and time[node] == 1:
                expect[3][1]["value"] = action["value"]
                expect[2][4]["call"]["value"] = action["value"]
                expect[1][4]["call"]["value"] = action["value"]

            assert action == expect[node][time[node]], "\naction=%s\nexpect=%s\nnode=%d\ntime=%d" % (action, expect[node][time[node]], node, time[node])
            time[node] += 1

            if node == 1 and time[node] == 6:
                loop += 1
                time = [-1, 0, 0, 0]
