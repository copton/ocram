...
-- critical_functions :: CallGraph -> BlockingFunctions -> Ast -> CriticalFunctions {{{1
critical_functions :: CallGraph -> BlockingFunctions -> Ast -> CriticalFunctions
critical_functions cg bf ast = foldl (travBlocking cg) Set.empty (Set.elems bf)
