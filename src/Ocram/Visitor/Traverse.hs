...
traverseCDeclMembers ((i1,i2,i3):os) d =
  ...
  in
    (o' ++ os', ut `mappend` u' `mappend` u1' `mappend` u2' `mappend` u3')

-- recurse {{{1
-- CTranslUnit {{{2
recurseCTranslUnit :: (DownVisitor d, UpVisitor d u, ListVisitor d u) => Recurse CTranslUnit d u
recurseCTranslUnit (CTranslUnit i x) d = let (i', u) = traverseCExtDecls i d in (CTranslUnit i' x, u)

-- CExtDecl {{{2
recurseCExtDecl :: (DownVisitor d, UpVisitor d u, ListVisitor d u) => Recurse CExtDecl d u
...
-- CDecl {{{2
recurseCDecl :: (DownVisitor d, UpVisitor d u, ListVisitor d u) => Recurse CDecl d u
recurseCDecl (CDecl x1 i x2) d = let (i', u) = traverseCDeclMembers i d in (CDecl x1 i' x2, u)

-- CDerivedDeclr {{{2
recurseCDerivedDeclr :: (DownVisitor d, UpVisitor d u, ListVisitor d u) => Recurse CDerivedDeclr d u
...
-- CFunDef {{{2
recurseCFunDef :: (DownVisitor d, UpVisitor d u, ListVisitor d u) => Recurse CFunDef d u
...
-- CDeclr {{{2
recurseCDeclr :: (DownVisitor d, UpVisitor d u, ListVisitor d u) => Recurse CDeclr d u
recurseCDeclr (CDeclr x1 i x2 x3 x4) d = let (i', u) = traverseCDerivedDeclrs i d in (CDeclr x1 i' x2 x3 x4, u)

-- CInit {{{2
recurseCInit :: (DownVisitor d, UpVisitor d u, ListVisitor d u) => Recurse CInit d u
...
-- CExpr {{{2
recurseCExpr :: (DownVisitor d, UpVisitor d u, ListVisitor d u) => Recurse CExpr d u
...
-- CStat {{{2
recurseCStat :: (DownVisitor d, UpVisitor d u, ListVisitor d u) => Recurse CStat d u
...
-- CBlockItem {{{2
recurseCBlockItem :: (DownVisitor d, UpVisitor d u, ListVisitor d u) => Recurse CBlockItem d u
...
maybeTrav travf (Just o) d = first Just $ travf o d
